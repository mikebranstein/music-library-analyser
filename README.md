...
- Score required-part completeness + tier, flag `score_missing` and `needs_review`, and record `detection_method`, `ocr_source`, `local_score_path`, `evidence_sources`, `candidate_score_images`, and resolved work identity (title, composer, arranger, publisher, catalog number, year, and a short 2–3 sentence `summary` of the piece)
...
9. `09_static_site.py`
- Transform the pipeline output (Scripts 01-08) into the data layer for a zero-build, offline web gallery (`web/`) that opens by double-clicking `index.html` (`file://`), so the collection can be browsed dashboard → pieces → documents → pages → the original PDF. A pure presentation transform: never re-derives completeness/quality/severity/instruments/reason codes, only reshapes the earlier facts into the shapes the site renders
...
project-root/
  ...
  web/
    index.html
    css/  js/  pages/
    data-sample/          # committed made-up fixture (copyright-safe preview)
    data/                 # generated by Script 09, git-ignored
    assets/
      thumbs/             # generated WebP thumbnails, git-ignored
      pdfs/               # optional bundled originals (--copy-pdfs), git-ignored
  cache/
    ocr/
    llm/
    render/
